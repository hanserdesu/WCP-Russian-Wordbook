...
function Download-WithProgress([string]$uri, [string]$destination, [string]$displayName, [int64]$expectedSize) {
    ...
    try {
        ...
        # 本地沙箱/镜像不应被机器上的代理再次转发。远程 GitHub 下载仍保留
        # 系统代理，以兼容需要 127.0.0.1:7897 的真实用户环境。
        $requestUri = [Uri]$uri
...
function Ensure-ZipExtractor {
    if ('WcpRussianZipSession' -as [type]) { return }
...
public sealed class WcpRussianZipSession
{
    public Task Task;
    ...
    public static WcpRussianZipSession Start(string zipPath, string destination, int workerCount)
    {
        WcpRussianZipSession session = new WcpRussianZipSession();
...
public static class WcpEs3Helper
{
    ...
    public static string FindBestSaveBackup(string dataDir)
    {
        ...
        string jpmodDir = Path.Combine(dataDir, "frmod_backups");
...
function Expand-ZipWithProgress([string]$zipPath, [string]$destination, [string]$displayName) {
    ...
    $session = [WcpRussianZipSession]::Start($zipPath, $destination, $workers)
...
$backup = Join-Path $data "frmod_backups\$stamp"
...
function Download-VerifiedAsset($asset) {
    ...
    $downloadDir = Join-Path $data 'frmod_downloads'
    ...
    if (Test-Path -LiteralPath $target) {
        ...
        if ($item.Length -eq [int64]$asset.size) {
            $valid = ((Get-Sha256Hex $target) -eq $asset.sha256.ToLower())
        }
    }
    if (-not $valid) {
        ...
        foreach ($baseUrl in $baseUrls) {
            try {
                ...
                if ($tmpItem.Length -eq [int64]$asset.size -and $tmpHash -eq $asset.sha256.ToLower()) {
                    Move-Item -LiteralPath $tmp -Destination $target -Force
                    $downloaded = $true
                    break
                }
                Write-Host "下载资源校验不匹配：$($asset.name)，实际大小 $($tmpItem.Length)，实际 SHA-256 $tmpHash" -ForegroundColor DarkYellow
            } catch {
                Write-Host "下载资源失败：$($asset.name)：$($_.Exception.Message)" -ForegroundColor DarkYellow
            }
            Remove-Item -LiteralPath $tmp -Force -ErrorAction SilentlyContinue
        }
...
$stagePattern = 'frmod_audio_stage*'
...
$audioStage = Join-Path $data ("frmod_audio_stage_{0}_{1}" -f $stamp, ([guid]::NewGuid().ToString('N')))
...
foreach ($name in @('RuWordListMod.dll', 'BookNameMod.dll', 'SentenceAudioRuMod.dll')) {
...
$repairDir = Join-Path $data 'ru_db_payload'
New-Item -ItemType Directory -Force -Path $repairDir | Out-Null
Get-ChildItem -LiteralPath (Join-Path $payload 'ru_db_payload') -File | ForEach-Object {
    Copy-Item -LiteralPath $_.FullName -Destination (Join-Path $repairDir $_.Name) -Force
}
# 俄语单词音频必须落进插件读取的私有目录 (RuWordListMod 拦截 VocabularyAudioPlayer
# 后从 ru_word_audio 播放)。绝不能写英语/日语共用的 LocalLow\WCP\vocabulary ——
# 8,116 词里 1,638 个与英语原库同形, 写进共享目录会让英/日语词书听到俄语发音。
New-Item -ItemType Directory -Force -Path (Join-Path $data 'ru_word_audio') | Out-Null
New-Item -ItemType Directory -Force -Path (Join-Path $data 'ru_sentence_audio') | Out-Null
Copy-TreeNet (Join-Path $audioStage 'vocabulary') (Join-Path $data 'ru_word_audio') 'words'
Copy-TreeNet (Join-Path $audioStage 'sentence_audio') (Join-Path $data 'ru_sentence_audio') 'sentences'
try {
...
$bookPayloadPath = Join-Path $payload 'catbar_russian_book.json'
if (-not (Test-Path -LiteralPath $bookPayloadPath)) { Fail '安装包缺少 catbar_russian_book.json。' }
...
# 2. 安全写入俄语词书与当前选中词书（完整保留全部存档元数据与 __type）
Write-Step '正在写入俄语词书并设置当前选中词书。'
if (Test-Path -LiteralPath $myBookPath) {
    $targetSlot = [WcpEs3Helper]::InstallBookAndSave($myBookPath, $savePath, $bookPayloadPath, 2)
    Write-Host "已写入俄语词书槽位 $targetSlot，已安全保留全部存档元数据并选中该词书。"
} else {
...
[IO.File]::WriteAllText((Join-Path $data 'frmod_install.json'), ($marker | ConvertTo-Json), (New-Object Text.UTF8Encoding($false)))
